...
# ===== RADIO PANEL =====
RADIO_CHANNEL_ID = 1495083810847457541
RADIO_ROLE_ID = 1495083809828503645
RADIO_FILE = "radio.json"
RADIO_COOLDOWN_SECONDS = 30
RADIO_TEMP_MESSAGE_DELETE_AFTER = 60
...
# RADIO SYSTEM
# =========================
def generate_radio_frequency() -> str:
    # Frekvence v rozsahu 1.99 až 999.99
...
def load_radio_state() -> dict:
    default_state = {
        "primary": generate_radio_frequency(),
        "secondary": generate_radio_frequency(),
        "active": "primary",
        "last_change": None,
        "message_id": None
    }
    ...
    if not os.path.exists(RADIO_FILE):
        save_radio_state(default_state)
    ...
    try:
        ...
        if data.get("active") not in ("primary", "secondary"):
            data["active"] = "primary"
...
def save_radio_state(data: dict):
    with open(RADIO_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4, ensure_ascii=False)


def radio_now_text() -> str:
    return discord.utils.format_dt(discord.utils.utcnow(), style="f")


def get_active_radio_label(state: dict) -> str:
    return "PRIMÁRNÍ" if state.get("active") == "primary" else "SEKUNDÁRNÍ"


def get_active_radio_frequency(state: dict) -> str:
    return state["primary"] if state.get("active") == "primary" else state["secondary"]


def build_radio_embed(guild: discord.Guild) -> discord.Embed:
    state = load_radio_state()
    active_label = get_active_radio_label(state)
    active_frequency = get_active_radio_frequency(state)

    embed = discord.Embed(
        title="📡 Vysílačka — panel",
        description=(
            f"## AKTUÁLNĚ JSTE NA:\n"
            f"# `{active_frequency}` MHz\n\n"
            f"**Aktivní vysílačka:** `{active_label}`\n\n"
            f"**Primární frekvence:** `{state['primary']}` MHz\n"
            f"**Sekundární frekvence:** `{state['secondary']}` MHz\n\n"
            f"**Použití**\n"
            f"Sledujte tento panel. Tady vždy poznáte, na jaké frekvenci jste aktuálně.\n\n"
            f"**Poslední změna:** {state.get('last_change') or 'Zatím žádná'}"
        ),
        color=discord.Color.blue() if state.get("active") == "primary" else discord.Color.red()
    )

    embed.set_author(
        name="New Generation Jalisco Cartel",
        icon_url=guild.icon.url if guild.icon else None
    )

    embed.set_footer(text="New Generation Jalisco Cartel")
    return embed


def radio_user_on_cooldown(user_id: int) -> int:
    now = discord.utils.utcnow().timestamp()
    last_used = radio_cooldowns.get(user_id, 0)
    remaining = RADIO_COOLDOWN_SECONDS - int(now - last_used)

    if remaining > 0:
        return remaining

    radio_cooldowns[user_id] = now
    return 0


async def send_temp_radio_message(channel: discord.TextChannel, content: str):
    try:
        await channel.send(content, delete_after=RADIO_TEMP_MESSAGE_DELETE_AFTER)
    except TypeError:
        msg = await channel.send(content)

        async def delete_later(message: discord.Message):
            await asyncio.sleep(RADIO_TEMP_MESSAGE_DELETE_AFTER)
            try:
...
class RadioView(discord.ui.View):
    ...
    @discord.ui.button(label="📡 Status", style=discord.ButtonStyle.primary, custom_id="radio_status")
    async def status_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        state = load_radio_state()
        await interaction.response.send_message(
            (
                f"📡 **AKTUÁLNĚ JSTE NA:** `{get_active_radio_frequency(state)}` MHz\n"
                f"**Aktivní vysílačka:** `{get_active_radio_label(state)}`\n"
                f"**Primární:** `{state['primary']}` MHz\n"
                f"**Sekundární:** `{state['secondary']}` MHz"
            ),
            ephemeral=True
        )

    @discord.ui.button(label="🔁 Přepnout", style=discord.ButtonStyle.secondary, custom_id="radio_switch")
    async def switch_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        remaining = radio_user_on_cooldown(interaction.user.id)
        if remaining > 0:
            await interaction.response.send_message(f"⏳ Počkej ještě {remaining}s.", ephemeral=True)
            return

        state = load_radio_state()
        state["active"] = "secondary" if state.get("active") == "primary" else "primary"
        state["last_change"] = radio_now_text()
        save_radio_state(state)

        await interaction.response.edit_message(embed=build_radio_embed(interaction.guild), view=RadioView())

        if isinstance(interaction.channel, discord.TextChannel):
            if state["active"] == "primary":
                await send_temp_radio_message(interaction.channel, "📡 **Přepnuto na PRIMÁRNÍ frekvenci!**")
            else:
                await send_temp_radio_message(interaction.channel, "📡 **Přepnuto na SEKUNDÁRNÍ frekvenci!**")

    @discord.ui.button(label="🔄 Změna frekvence", style=discord.ButtonStyle.success, custom_id="radio_change")
    async def change_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        state = load_radio_state()
        state["primary"] = generate_radio_frequency()
        state["secondary"] = generate_radio_frequency()
        ...
        if isinstance(interaction.channel, discord.TextChannel):
            await send_temp_radio_message(interaction.channel, "🔄 **Frekvence změněny — přepnuto na PRIMÁRNÍ frekvenci!**")

    @discord.ui.button(label="🚨 PANIC", style=discord.ButtonStyle.danger, custom_id="radio_panic")
    async def panic_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if isinstance(interaction.channel, discord.TextChannel):
            await send_temp_radio_message(
                interaction.channel,
                f"🚨 **PANIC! Přepnuto na SEKUNDÁRNÍ frekvenci!** <@&{RADIO_ROLE_ID}>"
            )
...
@bot.tree.command(name="sendcartel", description="Pošle New Generation Jalisco Cartel lore do určeného kanálu.", guild=guild_obj)
async def sendcartel_slash(interaction: discord.Interaction):
    ...
    part1 = (
        ...
        "New Generation Jalisco Cartel je označení pro staré korejské pouliční gangy a organizovaný zločin, který vznikal v temných částech Soulu, Busanu a Incheonu. "
        ...
        "New Generation Jalisco Cartel nikdy nebyl o zbytečném hluku. "
    ...
    part2 = (
        ...
        "Když se objeví problém, New Generation Jalisco Cartel ho vyřeší rychle, čistě a bez zbytečných otázek."
    )

    part3 = (
        "Dnes je New Generation Jalisco Cartel v Los Santos známé jméno mezi lidmi, kteří vědí, kam se dívat.\n\n"
    ...
    try:
        ...
        await send_embed_log(
            ...
            guild=interaction.guild,
            main_text=(
                f"📨 **New Generation Jalisco Cartel Lore Sent**\n"
                f"📍 **Channel:** {channel.mention}"
            ),
        ...
        await interaction.followup.send("✅ New Generation Jalisco Cartel lore bylo odesláno.", ephemeral=True)
...
# RADIO COMMAND
# =========================
@bot.tree.command(name="radio", description="Pošle nebo obnoví panel vysílačky.", guild=guild_obj)
async def radio_slash(interaction: discord.Interaction):
    ...
    state = load_radio_state()
    embed = build_radio_embed(interaction.guild)
    view = RadioView()
    ...
    if old_message_id:
        try:
            ...
            await message.edit(content=f"<@&{RADIO_ROLE_ID}>", embed=embed, view=view)
        except Exception:
            message = None

    if message is None:
        message = await channel.send(content=f"<@&{RADIO_ROLE_ID}>", embed=embed, view=view)

    state["message_id"] = message.id
    save_radio_state(state)

    await interaction.followup.send(f"✅ Radio panel je aktivní v {channel.mention}.", ephemeral=True)
