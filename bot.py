...
RULES_CHANNEL_ID = 1506345297285218305
APP_INFO_CHANNEL_ID = 1506345206952366222
ROLES_TEXT_CHANNEL_ID = 1506328425034678322
...
TASK_GROUPS = {
    1: {
        "channel_id": 1497989825565822988,
        "role_id": 1503145830100500721,
        "name": "GROUP 1",
    },
    2: {
        "channel_id": 1497989825565822989,
        "role_id": 1503145891375087698,
        "name": "GROUP 2",
    },
    3: {
        "channel_id": 1506258305519980575,
        "role_id": 1503145935373336777,
        "name": "GROUP 3",
    },
}

TASK_LEADER_ROLE_ID = 1506390581004206291
...
def build_meeting_embed(
    ...
    embed.set_footer(text="Meeting attendance • New Generation Jalisco Cartel")
    return embed
...
async def notify_leaders_about_submission(guild: discord.Guild, task_id: str, member: discord.Member):
    ...
    leader_role = guild.get_role(TASK_LEADER_ROLE_ID)
    if not leader_role:
        print("❌ Vedení role pro task systém nebyla nalezena.")
        return
    ...
    embed.set_footer(text="Schválit může pouze vedení.")
    view = TaskApprovalView(task_id, member.id)

    sent = 0
    for leader in leader_role.members:
        if leader.bot:
            continue
        try:
            ...
            print(f"❌ Nepodařilo se poslat PM vedení: {e}")

    if sent == 0:
        print("⚠️ PM vedení se neposlala nikomu. Možná mají vypnuté DMs.")
...
class TaskApprovalView(discord.ui.View):
    ...
    async def check_leader(self, interaction: discord.Interaction) -> bool:
        ...
        if not leader_role or leader_role not in member.roles:
            await interaction.response.send_message("❌ Nemáš oprávnění schvalovat tasky.", ephemeral=True)
            return False
