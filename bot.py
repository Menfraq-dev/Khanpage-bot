...
def generate_radio_frequency() -> str:
    value = random.uniform(1.99, 999.99)
    return f"{value:.2f}"


def build_radio_embed(primary: str, secondary: str, changed_by: discord.abc.User | None = None) -> discord.Embed:
    now = discord.utils.utcnow()

    embed = discord.Embed(
        title="📡 Vysílačka — Primární",
        description=(
            f"**Primární frekvence:** `{primary}` MHz\n"
            f"**Sekundární frekvence:** `{secondary}` MHz\n\n"
            f"**Použití**\n"
            f"Přepněte všichni na tyto frekvence.\n\n"
            f"**Poslední změna:** {discord.utils.format_dt(now, style='f')}"
            + (f"\n**Změnil:** {changed_by.mention}" if changed_by else "")
        ),
...
class RadioView(discord.ui.View):
    ...
    async def status_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "📡 Aktuální frekvence jsou napsané v panelu nad tlačítky.",
            ephemeral=True
        )

    @discord.ui.button(label="Změna frekvence", style=discord.ButtonStyle.secondary, emoji="🔄")
    async def change_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = build_radio_embed(primary, secondary, interaction.user)

        await interaction.response.edit_message(
            content=f"<@&{RADIO_ROLE_ID}>",
            embed=embed,
            view=self
        )

    @discord.ui.button(label="PANIC", style=discord.ButtonStyle.danger, emoji="🚨")
    async def panic_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            f"🚨 **PANIC!** <@&{RADIO_ROLE_ID}> okamžitě zkontrolujte vysílačku!",
...
# RADIO COMMAND
# =========================
@bot.tree.command(name="radio", description="Pošle panel s aktuální frekvencí vysílačky.", guild=guild_obj)
async def radio_slash(interaction: discord.Interaction):
    ...
    if last_used:
        remaining = RADIO_COOLDOWN_SECONDS - (now - last_used).total_seconds()
        if remaining > 0:
            await interaction.response.send_message(
                f"⏳ Tento command můžeš použít znovu za **{int(remaining)} sekund**.",
                ephemeral=True
            )
            return
    ...
    try:
        ...
        await interaction.response.send_message("❌ Nemám oprávnění poslat zprávu do radio kanálu.", ephemeral=True)
    except Exception as e:
        await interaction.response.send_message(f"❌ Chyba při odesílání radio panelu: {e}", ephemeral=True)
