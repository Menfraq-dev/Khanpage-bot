...
def generate_radio_frequency() -> str:
    # rozsah 1.99 až 999.99, vždy dvě desetinná místa
    value = random.randint(199, 99999) / 100
    return f"{value:.2f}"


@bot.tree.command(name="radio", description="Vygeneruje novou náhodnou frekvenci vysílačky.", guild=guild_obj)
async def radio_slash(interaction: discord.Interaction):
    ...
    last_used = radio_cooldowns.get(user_id)

    if last_used:
        remaining = RADIO_COOLDOWN_SECONDS - int((now - last_used).total_seconds())
        if remaining > 0:
            await interaction.response.send_message(
                f"⏳ Tento command můžeš použít znovu za **{remaining} s**.",
                ephemeral=True
            )
            return
    ...
    role = interaction.guild.get_role(RADIO_ROLE_ID)
    role_ping = role.mention if role else f"<@&{RADIO_ROLE_ID}>"
    frequency = generate_radio_frequency()

    embed = discord.Embed(
        title="📻 Nová frekvence vysílačky",
    ...
    embed.add_field(
        name="Použití",
        value="Přepněte všichni na tuto frekvenci.",
        inline=False
    )

    embed.set_footer(text=f"Vygeneroval: {interaction.user.display_name}")
    ...
    await interaction.response.send_message(
        f"✅ Nová frekvence `{frequency}` byla odeslána do {channel.mention}.",
        ephemeral=True
    )
...
async def sendcartel_slash(interaction: discord.Interaction):
    ...
    part1 = (
        ...
        "New Generation Jalisco Cartel je organizovaná frakce s pevnou hierarchií, disciplínou a důrazem na loajalitu. "
        ...
        "New Generation Jalisco Cartel nikdy není o zbytečném hluku. "
