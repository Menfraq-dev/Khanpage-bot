...
# ===== SERVER =====
GUILD_ID = 1483033498171211862

# ===== HLAVNÍ ROLE PRO VĚTŠINU COMMANDŮ =====
ALLOWED_COMMAND_ROLE_ID = 1483044202119172126

# ===== SPECIÁLNÍ ROLE PRO VYBRANÉ COMMANDY =====
SPECIAL_COMMAND_ROLE_ID = 1483175786067398870

# ===== KANÁLY =====
WELCOME_CHANNEL_ID = 1483945228111974554
RULES_CHANNEL_ID = 1483185971930468555
LOG_CHANNEL_ID = 1483945025195999302
PUNISH_LOG_CHANNEL_ID = 1483945290515087360
REVERT_LOG_CHANNEL_ID = 1483194779041923277
STRIKE_CHANNEL_ID = 1483867641964658718
APP_INFO_CHANNEL_ID = 1483186262923018401
LA_TERRAZA_TEXT_CHANNEL_ID = 1483186090406969404
ROLES_TEXT_CHANNEL_ID = 1483041770857304075

# ===== MESSAGE LOG CHANNEL =====
MESSAGE_LOG_CHANNEL_ID = 1489216094286909531
...
# ===== STABILITA / WATCHDOG =====
last_ready_timestamp = 0.0
last_gateway_event_timestamp = time.time()
...
@bot.event
async def on_ready():
    global last_ready_timestamp, last_gateway_event_timestamp

    last_ready_timestamp = time.time()
    last_gateway_event_timestamp = time.time()
    ...
    if not gateway_watchdog.is_running():
        gateway_watchdog.start()


@bot.event
async def on_connect():
    print("🔌 Bot se připojuje k Discordu...")


@bot.event
async def on_disconnect():
    print("⚠️ Bot se odpojil od Discord gateway.")


@bot.event
async def on_resumed():
    global last_gateway_event_timestamp
    last_gateway_event_timestamp = time.time()
    print("♻️ Discord session byla obnovena.")
...
async def gateway_watchdog():
    global last_gateway_event_timestamp, last_ready_timestamp

    if not bot.is_ready():
        return

    now = time.time()
    seconds_since_gateway = now - last_gateway_event_timestamp

    if seconds_since_gateway > 600:
        print(f"❌ Watchdog: žádný gateway event {int(seconds_since_gateway)}s. Restartuji proces...")
        os._exit(1)

    ws = getattr(bot, "ws", None)
    if ws is None:
        print("❌ Watchdog: websocket není dostupný. Restartuji proces...")
        os._exit(1)

    latency = round(bot.latency * 1000) if bot.latency is not None else -1
    print(f"💓 Watchdog OK | latency={latency}ms | gateway idle={int(seconds_since_gateway)}s")


@gateway_watchdog.before_loop
    ...
    try:
        if interaction.response.is_done():
            await interaction.followup.send(
                "❌ Command spadl na chybě. Mrkni do Render logů.",
                ephemeral=True
            )
        else:
        ...
        print(f"❌ Nepodařilo se poslat error response: {e}")
